...
import com.cart.cart.dao.BillDao;
    ...
    public ResponseEntity<String> generateReport(Map<String, Object> requestMap) {
        ...
        try{
            ...
            if (validateRequestMap(requestMap)) {
                ...
                PdfWriter.getInstance(document, new FileOutputStream(CartConstants.STORE_LOCATION+"\\" + fileName+".pdf"));

                document.open();
                setRectangleInPdf(document);

                Paragraph chunk = new Paragraph("Canteen Management System", getFont("Header"));
                ...
                JSONArray jsonArray = CartUtils.getJSONArrayFromString((String) requestMap.get("productDetails"));
                for(int i = 0; i < jsonArray.length(); i++){
                    addRows(table, CartUtils.getMapFromJson(jsonArray.getString(i)));
                }
                document.add(table);
            ...
            return CartUtils.getResponseEntity("Required data not found", HttpStatus.BAD_REQUEST);
        }catch(Exception ex){
            ex.printStackTrace();
        }
